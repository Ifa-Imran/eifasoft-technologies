...
import { useP2P24hStats } from '@/hooks/useP2P24hStats';
...
export default function ExchangePage() {
  ...
  const { trades24h: p2pFilledTrades, volume24h: p2pTotalVolume } = useP2P24hStats();
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {priceVal > 0 && (
          <div className="rounded-2xl px-5 py-3 bg-gradient-to-r from-primary-100 to-secondary-100 border-2 border-primary-300/50 flex items-center gap-3 shadow-sm shadow-primary-200/20">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-wider text-surface-400">{t('dexPrice')}</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        <GlassCard padding="p-4" variant="cyan">
          <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('buyOrders')}</p>
        ...
        <GlassCard padding="p-4" variant="purple">
          <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('sellOrders')}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <GlassCard>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-success-600">{t('buyOrders')}</h3>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {activeBuyOrders.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-surface-400 text-sm">{t('noActiveBuyOrders')}</p>
                  </div>
                ) : (
            ...
            <GlassCard>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-danger-500">{t('sellOrders')}</h3>
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto">
                {activeSellOrders.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-surface-400 text-sm">{t('noActiveSellOrders')}</p>
                  </div>
                ) : (
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                <label className="text-sm font-medium text-surface-600">
                  {orderType === 'buy' ? t('usdtAmount') : t('kairoAmount')}
                </label>
                <button
                  onClick={() => {
                ...
                >
                  {t('max', { amount: orderType === 'buy' ? Number(usdtFormatted).toFixed(2) : Number(kairoFormatted).toFixed(2) })}
                </button>
              </div>
              <input
                type="number"
              ...
              <p className="mt-1 text-xs text-surface-400">
                {orderType === 'buy' ? t('balanceLabel', { amount: Number(usdtFormatted).toFixed(2), token: 'USDT' }) : t('balanceLabel', { amount: Number(kairoFormatted).toFixed(2), token: 'KAIRO' })}
              </p>
            </div>
            ...
            {numAmount > 0 && (
              <div className="mt-4 p-4 rounded-xl bg-white/60 border border-surface-200 space-y-2 text-xs">
                ...
                <div className="flex justify-between font-semibold text-surface-900 border-t border-surface-200 pt-2">
                  <span>{t('estimated', { token: orderType === 'buy' ? 'KAIRO' : 'USDT' })}</span>
          ...
          {myBuyOrders.length === 0 && mySellOrders.length === 0 ? (
            <GlassCard>
              <div className="text-center py-12">
                ...
                <p className="text-surface-500 text-sm">{t('noActiveOrders')}</p>
                <p className="text-surface-400 text-xs mt-1">{t('noActiveOrdersDesc')}</p>
              </div>
            </GlassCard>
          ) : (
            <>
              {myBuyOrders.length > 0 && (
                <GlassCard>
                  <h3 className="text-sm font-semibold text-success-600 mb-3 flex items-center gap-2">
                    <ShoppingCartIcon className="w-4 h-4" /> {t('myBuyOrders')}
                  </h3>
                          ...
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm font-mono font-semibold text-surface-900">
                                {t('remaining', { amount: usdtVal.toFixed(2) })}
                              </p>
                              <p className="text-xs text-surface-400">
                                {t('filledOfTotal', { filled: filledUsdt.toFixed(2), total: totalUsdt.toFixed(2) })}
                              </p>
                            </div>
              ...
              {mySellOrders.length > 0 && (
                <GlassCard>
                  <h3 className="text-sm font-semibold text-danger-500 mb-3 flex items-center gap-2">
                    <TagIcon className="w-4 h-4" /> {t('mySellOrders')}
                  </h3>
                  <div className="space-y-2">
                      ...
                      return (
                        <div key={i} className="p-3 rounded-xl bg-danger-50/40 border border-danger-100 space-y-2">
                          <div className="flex items-center justify-between">
                            <div>
                              <p className="text-sm font-mono font-semibold text-surface-900">
                                {t('kairoRemaining', { amount: kairoVal.toFixed(2) })}
                              </p>
                              <p className="text-xs text-surface-400">
                                {t('filledOfTotal', { filled: filledKairo.toFixed(2), total: totalKairo.toFixed(2) })}
                              </p>
                            </div>
