...
export default function RankDividendPage() {
  ...
  const t = useTranslations('rankDividend');
  const { allIncome, rankInfo, directReferrals, teamVolume, unlockedLevels, legVolumes, largestLegVolume, harvestIncome, checkRankChange, storedRank, liveRank, rankSalary, nextRankClaim, isRankChangePending, pendingRankSalary: pendingRankSalaryBn, totalRankHarvestable, teamAnalytics, activeDirects, isLoading, isPending } = useAffiliate();
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 p-4 rounded-2xl bg-gradient-to-r from-warn-50 to-warn-100 border border-warn-300 shadow-sm">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-warn-800">
              {t('rankChangeDetected', { from: RANK_NAMES[storedRank], to: RANK_NAMES[liveRank] })}
            </p>
      ...
      <GlassCard variant="gradient" padding="p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="text-center p-3 sm:p-4 rounded-2xl bg-white/60 border-2 border-primary-200/50 shadow-sm overflow-hidden">
            ...
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('currentRank')}</p>
            <p className="text-base sm:text-xl md:text-2xl font-orbitron font-bold gradient-text truncate">{rankName}</p>
            <Badge tier="purple" size="sm">{t('rankLevel', { level: currentRank })}</Badge>
          </div>
          ...
          <div className="text-center p-3 sm:p-4 rounded-2xl bg-white/60 border-2 border-accent-200/50 shadow-sm overflow-hidden">
            ...
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('rankSalary')}</p>
            ...
            <p className="text-[10px] text-surface-400">{t('perPeriod')}</p>
          </div>

          {/* Harvestable Salary */}
          <div className="text-center p-3 sm:p-4 rounded-2xl bg-white/60 border-2 border-success-200/50 shadow-sm overflow-hidden col-span-2 md:col-span-1">
            ...
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('harvestable')}</p>
            <p className="text-base sm:text-xl md:text-2xl font-mono font-bold text-success-700 truncate">
              ${harvestableRankUsd.toFixed(2)}
            </p>
            <p className="text-[10px] text-surface-400">{t('crystallized')}</p>
          </div>
        </div>
          ...
          <div className="flex items-center justify-between p-3 rounded-xl bg-white/50 border border-surface-200 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-surface-600">
                {t('earningPerPeriod', { amount: rankSalary ? Number(formatUnits(rankSalary, USDT_DECIMALS)).toFixed(2) : '0' })}
              </span>
            </div>
            <p className="text-xs text-surface-400">
              {t('pending', { amount: pendingRankUsd.toFixed(2) })}
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          >
            {isRankChangePending
              ...
              : pendingRankSalaryBn > 0n
                ? t('syncRankSalary', { amount: pendingRankUsd.toFixed(2) })
                : t('nothingToSync')}
          </Button>
          <Button
          ...
          >
            {harvestableRank > 0n
              ? t('harvestBtn', { amount: harvestableRankUsd.toFixed(2) })
              : t('nothingToHarvest')}
          </Button>
        </div>
      </GlassCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <GlassCard>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-surface-900">{t('teamVolumeTitle')}</h3>
          </div>
          <div className="text-center p-5 rounded-2xl bg-gradient-to-br from-primary-50/60 to-white/60 border-2 border-primary-200/50 mb-4">
            ...
            <p className="text-sm text-surface-500 mt-1">{t('totalTeamVolume')}</p>
          </div>

          {currentRank < 10 && (
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-surface-500">{t('next', { rank: RANK_NAMES[currentRank + 1] })}</span>
                <span className="font-mono font-semibold text-primary-600">
                  ${formatCompact(nextThreshold, 2)}
                </span>
              </div>
              <ProgressBar value={progressToNext} label={t('progressAchieved', { percent: progressToNext.toFixed(2) })} variant="cyan" size="lg" />
              <p className="text-xs text-surface-400 mt-1">
                {nextQualifyingVol >= nextThreshold
                  ? t('targetMet')
                  : t('needMoreVolume', { amount: formatCompact(nextThreshold - nextQualifyingVol, 2) })}
              </p>
            </div>
          )}
          ...
          <div className="mt-4 grid grid-cols-3 gap-2">
            ...
            <div className="p-3 rounded-xl bg-gradient-to-r from-success-100 to-success-50 border border-success-200/50 text-center">
              <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('activeDirects')}</p>
        ...
        <GlassCard>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-surface-900">{t('harvestIncomeTitle')}</h3>
          </div>
          <div className="space-y-2">
            {[
              { key: 'direct', color: 'bg-primary-500', text: 'text-primary-600' },
              { key: 'team', color: 'bg-secondary-500', text: 'text-secondary-600' },
              { key: 'rank', color: 'bg-accent-500', text: 'text-accent-600' },
            ].map((item, i) => {
              ...
              return (
                <div key={i} className="flex items-center justify-between p-2.5 rounded-xl bg-gradient-to-r from-surface-50 to-primary-50/30 border border-primary-100/30 hover:border-primary-200 transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-surface-600">{t(item.key)}</span>
      ...
      <GlassCard>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-surface-900">{t('fiftyPctCapRule')}</h3>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          <div className="p-3 rounded-xl bg-gradient-to-br from-primary-50 to-primary-100/60 border border-primary-200/50 text-center">
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('totalTeamVolumeLabel')}</p>
          ...
          <div className="p-3 rounded-xl bg-gradient-to-br from-warn-50 to-warn-100/60 border border-warn-200/50 text-center">
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('largestLeg')}</p>
          ...
          <div className="p-3 rounded-xl bg-gradient-to-br from-secondary-50 to-secondary-100/60 border border-secondary-200/50 text-center">
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('numberOfLegs')}</p>
          ...
          <div className="p-3 rounded-xl bg-gradient-to-br from-success-50 to-success-100/60 border border-success-200/50 text-center">
            <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('qualifyingVolume')}</p>
        ...
        {legVolumeNumbers.length > 0 && (
          <div className="space-y-2">
            <p className="text-xs font-semibold text-surface-600 mb-1">{t('legVolumeBreakdown')}</p>
            {legVolumes.map((leg: any, i: number) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-xs">
                  <span className="w-16 text-surface-500 flex-shrink-0">{t('leg', { index: i + 1 })}</span>
                  ...
                  {isCapped && (
                    <span className="text-warn-600 text-[10px] flex-shrink-0">{t('maxCredited', { amount: formatCompact(credited, 2) })}</span>
                  )}
                </div>
              );
            })}
      ...
      <GlassCard>
        <h3 className="text-lg font-semibold text-surface-900 mb-4">{t('rankTargetsTitle')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-200">
                <th className="text-left py-3 px-3 text-surface-500 font-medium">{t('rankCol')}</th>
                <th className="text-right py-3 px-3 text-surface-500 font-medium">{t('teamVolumeTargetCol')}</th>
                <th className="text-right py-3 px-3 text-surface-500 font-medium">{t('salaryCol')}</th>
                <th className="text-center py-3 px-3 text-surface-500 font-medium">{t('progressCol')}</th>
                <th className="text-center py-3 px-3 text-surface-500 font-medium">{t('statusCol')}</th>
              </tr>
            </thead>
            <tbody>
