  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-base text-surface-500 mt-1">{t('subtitle', { fee: feePercent })}</p>
      </div>
      ...
      <div className="max-w-lg mx-auto space-y-4">
        ...
        <GlassCard padding="p-4 sm:p-5" variant="gold" className="w-full">
          <div className="flex items-center justify-between">
            <p className="text-xs sm:text-sm uppercase tracking-wider text-surface-400">{t('poolUsdt')}</p>
        ...
        <GlassCard variant="gradient">
          <div className="mb-6 text-center">
            ...
            <p className="text-sm text-surface-500">{t('kairoToUsdt')}</p>
            <p className="text-xs text-surface-400">{t('swapBurnNote')}</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-primary-50/60 to-white/70 border-2 border-primary-200/40 hover:border-primary-300 transition-colors">
              <div className="flex justify-between mb-2">
                <span className="text-xs text-surface-500">{t('youPay')}</span>
                <button
                  onClick={() => {
                ...
                >
                  {t('max', { amount: Number(kairoFormatted).toFixed(2) })}
                </button>
              </div>
            ...
            <div className="p-4 rounded-xl bg-gradient-to-r from-accent-50/60 to-white/70 border-2 border-accent-200/40">
              <div className="flex justify-between mb-2">
                <span className="text-xs text-surface-500">{t('youReceive')}</span>
              </div>
              <div className="flex items-center gap-2">
              ...
              <div className="p-4 rounded-xl bg-white/60 border border-surface-200 space-y-2 text-xs">
                ...
                <div className="flex justify-between text-surface-500">
                  <span className="flex items-center gap-1">
                    <FireIcon className="w-3 h-3 text-danger-400" /> {t('fee', { percent: feePercent })}
                  </span>
                  <span className="font-mono">${formatCompact(fee, 2)}</span>
                </div>
                <div className="flex justify-between text-surface-500">
                  <span>{t('minOutput', { slippage })}</span>
                ...
                {priceImpact > 0.01 && (
                  <div className={`flex justify-between ${priceImpact > 5 ? 'text-danger-500 font-semibold' : 'text-surface-500'}`}>
                    <span className="flex items-center gap-1">
                      <InformationCircleIcon className="w-3 h-3" /> {t('priceImpact')}
                    </span>
            ...
            {/* Slippage */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-surface-500">{t('slippage')}</span>
              {['0.5', '1', '2'].map((s) => (
                <button
                  key={s}
            ...
            <Button
              ...
              className="w-full"
            >
              {needsApproval ? t('approveAndSwap') : t('swapBtn')}
            </Button>
          </div>
        </GlassCard>
      </div>
