...
export function ReferralWidget() {
  ...
  return (
    <GlassCard>
      <h3 className="text-xl font-semibold text-surface-900 mb-5">{t('title')}</h3>
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="p-4 rounded-xl bg-gradient-to-br from-secondary-200 to-secondary-100 border-2 border-secondary-300/50 text-center shadow-sm shadow-secondary-200/30">
          <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-1">{t('rank')}</p>
        ...
        <div className="p-4 rounded-xl bg-gradient-to-br from-primary-200 to-primary-100 border-2 border-primary-300/50 text-center shadow-sm shadow-primary-200/30">
          ...
          <p className="text-xl font-mono font-bold text-primary-700">
            {referralCount}
            {teamAnalytics && (
              <span className="text-xs font-normal text-success-600 ml-1">({t('directActive', { count: teamAnalytics.directActive })})</span>
            )}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-5">
        <div className="p-3 rounded-lg bg-surface-50 border border-surface-200 text-center">
          <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-0.5">{t('directBusiness')}</p>
          ...
          <p className="text-[10px] text-surface-400">{t('referralStakes')}</p>
        </div>
        <div className="p-3 rounded-lg bg-surface-50 border border-surface-200 text-center">
          <p className="text-[10px] uppercase tracking-wider text-surface-400 mb-0.5">{t('teamBusiness')}</p>
          <p className="text-lg font-mono font-bold text-accent-700">
            ${formatCompact(teamVolumeUsd, 2)}
          </p>
          <p className="text-[10px] text-surface-400">{t('totalDownlineVolume')}</p>
        </div>
      </div>
